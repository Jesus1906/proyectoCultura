   ...
   $.ajax({
      ...
      success:function(response){
         ...
         tablas.forEach(tabla =>{
            plantilla+=`
            <tr valor = "${tabla.Profesor_Matricula}" class = "editarValor">
...
function consultar(id){
   $.ajax({
      ...
      success:function(response){
         ...
         tablas.forEach(tabla =>{
            plantilla+=`
            <div class="row" id = "row" valor = "${tabla.Profesor_Matricula}">
                <div class="col-sm-6">
            ...
            <div class="col-sm-6">
             <div class="form-group">
                ...
                <input type="tel" class="form-control" id="phone" name="phone" data-toggle="tooltip" data-placement="left" title="Inserta tu número en el siguiente formato: 55-11-22-33-44" pattern="[0-9]{2}-[0-9]{2}-[0-9]{2}-[0-9]{2}-[0-9]{2}"value = "${tabla.phone}">
             </div>
            </div>
            <div class="col-sm-6">
             <div class="form-row">
                <label for="email">Correo Electronico</label>
                <input type="email" class="form-control" id="email" name="email" value ="${tabla.email}">
             </div>
            </div>
            `;
...
$('#formUsuarios').submit(function(e){
   let form = {
      Profesor_Matricula: $('#row').attr('valor'),
   ...
   $.post('../../ajaxProf/editar', form, function(response){
      console.log(response);
